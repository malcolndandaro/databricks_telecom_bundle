...
select 
  ...
  FL_PLNO_TTLR
from
  STREAM(${confs.p_catalog}.customer_silver.product_subscriptions)
where
  __END_AT is null
...
from
  ...
  join ${confs.p_catalog}.misc.dim_date as dim_date on product_subscriptions.dt_ini_plno = dim_date.DATA
  join ${confs.p_catalog}.customer_gold.dim_customer as dim_customer on product_subscriptions.nu_doct = dim_customer.nu_doct
  and product_subscriptions.nu_tlfn = dim_customer.nu_tlfn
  and dim_customer.__end_at is null
  join ${confs.p_catalog}.customer_gold.dim_product as dim_product on product_subscriptions.id_prdt = dim_product.id_prdt
